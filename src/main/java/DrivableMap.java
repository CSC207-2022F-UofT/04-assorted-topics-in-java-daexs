...
class DrivableMap {
    ...
    public boolean hasFasterThan(int speed){
        Collection<Drivable> allValues = drivable_map.values();
        for(Drivable s : allValues){
    ...
    public List<Tradable> getTradable(){
        Collection<Drivable> allValues = drivable_map.values();
        List<Tradable> allTradables = new ArrayList<Tradable>();
        for (Drivable s : allValues){
            if (s instanceof Tradable){
                allTradables.add(0, (Tradable) s);
            }
        }
        return allTradables;
    }
